...
| **LedgerTable** | `components/data/LedgerTable.tsx` | Re-export of shadcn Table primitives with usage convention comments | Same as Table primitives | Thin re-export + conventions | Documented in-file comments | Usage: `text-end` for numeric cells, `className="interactive"` for clickable rows (replaces `cursor-pointer hover:bg-accent` — see D14 and design-system.md §3a) | §5 Table |
...
| **OrderDrawer** | `components/orders/OrderDrawer.tsx` | Right-side detail drawer for a single order | `order: Order \| null`, `onOpenChange: (open: boolean) => void` | Custom (wraps Sheet, Timeline, OrderStatusChip, formatMoney) | open (order !== null), closed | `SheetContent side="right"` — physical; reads i18n via `useTranslations("orders")`; single `SheetClose` in the header — shadcn's built-in absolute-positioned × was removed to avoid duplicate close targets (see design-system.md §3b) | §4.8 |
...
| **ThemeProvider** | `components/theme/ThemeProvider.tsx` | Wraps `next-themes` `ThemeProvider`; applies `.dark` class to `<html>` based on resolved theme | `children: React.ReactNode` | Custom (thin wrapper over `next-themes`) | system (default), light, dark | No direction dependency; must be an ancestor of any component using `useTheme` | D13 |
| **ThemeToggle** | `components/shell/ThemeToggle.tsx` | Compact toggle button that switches between light and dark theme; renders a Sun icon in dark mode and Moon icon in light mode | No external props; reads `useTheme()` from `next-themes` and `useTranslations("common")` | Custom | light (Moon icon), dark (Sun icon), SSR placeholder (disabled, same dimensions, avoids hydration mismatch via `useSyncExternalStore`) | Inherits logical layout; no direction dependency | D13 |
